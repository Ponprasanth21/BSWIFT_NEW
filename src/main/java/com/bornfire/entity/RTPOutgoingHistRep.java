...
public interface RTPOutgoingHistRep extends JpaRepository<RTP_Outgoing_hist_entity, String> {
	...
	@Query(value = "select * from BIPS_OUTWARD_TRANSACTION_HIST_MONITORING_TABLE where (msg_type='OUTGOING') and  trunc(tran_date)=?1 and tran_date is not null order by tran_date desc ", nativeQuery = true)
	List<RTP_Outgoing_hist_entity> findAllCustomoutward(String tranDate);
	
	@Query(value = "select * from BIPS_OUTWARD_TRANSACTION_HIST_MONITORING_TABLE where (msg_type='OUTWARD_BULK_RTP') and  trunc(tran_date)=?1 and tran_date is not null order by tran_date desc ", nativeQuery = true)
	List<RTP_Outgoing_hist_entity> findAllCustomoutwardrtp(String tranDate);
	
	@Query(value = "select cim_account,cim_account_name,ipsx_account,ipsx_account_name,tran_date,value_date,sequence_unique_id,tran_audit_number,msg_type,tran_amount,entry_user,master_ref_id,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=cdtr_agt)cdtr_agt,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=dbtr_agt)dbtr_agt,init_channel_id,tran_currency,tran_status,cbs_status,cbs_status_error,response_status,ipsx_status_error,resv_field1 from BIPS_OUTWARD_TRANSACTION_HIST_MONITORING_TABLE where (msg_type='OUTWARD_BULK_RTP') and  trunc(tran_date)=?1 and tran_date is not null order by tran_date desc ", nativeQuery = true)
	...
	@Query(value = "select * from BIPS_OUTWARD_TRANSACTION_HIST_MONITORING_TABLE where master_ref_id=?1 and  trunc(tran_date)=?2", nativeQuery = true)
